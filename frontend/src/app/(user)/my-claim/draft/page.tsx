...
export default function DraftClaimsPage() {
  ...
  const [cancelDiaglogOpen, setCancelDiaglogOpen] = useState(false);
  ...
  const { data, isLoading } = useQuery<Claim[]>({
    ...
    queryFn: async () => {
      try {
        const response = await claimApi.myClaim("draft");
  ...
  const handleCancel = (claim: Claim) => {
    setSelectedClaim(claim);
    setCancelDiaglogOpen(true);
  };
  ...
  const handleCancelSubmit = async () => {
    ...
    try {
      const response = await claimApi.cancelClaim(selectedClaim.id);
      if (response.isSuccess) {
        toast({
          title: "Success",
          description: response.message,
  ...
  const columns = createColumns(handleViewDetail, handleEdit, handleSubmit, handleCancel);
  ...
  return (
    <div className="container mx-auto py-10 px-8">
      <div className="mb-8">
        <h1 className="text-3xl font-bold tracking-tight">Draft Claims</h1>
      ...
      <AlertDialog open={cancelDiaglogOpen} onOpenChange={setCancelDiaglogOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
  ...
  Please click ‘OK’ to cancel the claim or ‘Cancel’ to close the dialog.
          ...
          <AlertDialogFooter>
            <AlertDialogCancel>Cancel</AlertDialogCancel>
            <AlertDialogAction onClick={handleCancelSubmit}>OK</AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
    </div>
